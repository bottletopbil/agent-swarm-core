...
Currently, the system is in **Phase 6.5**. It operates as a locally-decoupled, asynchronous framework orchestrated by a central Coordinator, writing to a persistent SQLite State Tracker.

It consists of 5 decoupled services communicating via a local message queue:
*   **Coordinator**: The orchestrator that routes `PENDING` and `LOCKED` tasks to the appropriate queue, managing DAG dependency resolution.
...
*   **Worker Agent**: Executes specific prompts against the LLM (currently OpenAI gpt-4o-mini).
...
*   **Observation Dashboard**: A live FastAPI/HTML dashboard at `localhost:8000` to visualize the Kanban state of the swarm and inspect the DAG.
...
1.  **NATS Event Bus**: Replacing local queues with JetStream pub/sub.
